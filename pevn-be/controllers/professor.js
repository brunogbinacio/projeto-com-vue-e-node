...
professor.readCourse = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            message: "Erro ao tentar listar os cursos",
            error,
        });
    }
};
...
professor.getDeliveries = async (req, res) => {
    ...
    try {
        const deliveries = await (
            await pool.query("SELECT * FROM delivery JOIN (SELECT id_s, s_name FROM student) AS S ON s_id=id_s WHERE a_id=$1", [id_a])
        ).rows;
        res.status(200).json(deliveries);
    } catch (error) {
        res.status(500).json({
            message: "an error has ocurrer",
            error,
        });
    }
};
