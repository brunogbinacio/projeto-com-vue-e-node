    <template>
        <v-container class="text-center">
            ...
            <v-alert text v-model="alert.show" :type="alert.type" dismissible class="text-start">{{ alert.message
            }}</v-alert>
                    ...
                    <v-simple-table fixed-header>
                        ...
                        <tbody>
                            <tr v-for="assignment in assignmentsList" :key="assignment.id_a">
                                ...
                                <td><v-icon @click="open(assignment.a_file)">mdi-file</v-icon></td>
                                <td v-if="assignment.d_file == null"><v-icon color="info"
                                        @click="openForm(assignment.id_a)">mdi-cloud-upload</v-icon></td>
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <v-form ref="addForm" class="ma-3" @submit.prevent="addDelivery">
                            <v-file-input label="Delivery" v-model="deliveryToAdd.d_file"
                                :rules="[(v) => !!v || 'Arquivo necessário']"></v-file-input>
