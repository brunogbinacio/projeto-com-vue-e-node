<template>
    <v-container class="text-center">
        ...
        <v-alert class="text-start" text v-model="alert.show" :type="alert.type" dismissible>{{ alert.message
        }}</v-alert>
        <h1 class="front-weight-light">{{ course.c_name }}</h1>
        <h2 class="front-weight-thin">{{ course.c_description }}</h2>
                ...
                <v-simple-table>
                    ...
                    <tbody>
                        <tr v-for="assignment in assignmentsList" :key="assignment.id_a">
                            ...
                            <td><v-icon @click="open(assignment.a_file)">mdi-file-image</v-icon></td>
                            <td><v-icon color="cyan darken-1"
                                    @click="getDeliveries(assignment.id_a)">mdi-book-open-page-variant</v-icon></td>
                        </tr>
                    </tbody>
                </v-simple-table>
            </v-col>
        ...
        <v-dialog v-model="showDelv" max-width="450">
            <v-simple-table fixed-header>
                <thead>
                    <tr>
                        <th text-center>Estudante</th>
                        <th text-center>Delivery</th>
                    </tr>
                </thead>
                <tbody>
                                ...
                                <v-card>
                                    ...
                                    <v-card-text>
                                        <v-form ref="addForm" class="ma-3" @submit.prevent="addAssignment()">
                                            <v-file-input label="File" :rules="[(v) => !!v || 'File is required']"
                                                v-model="assignmentToAdd.a_file"></v-file-input>
                                            <v-text-field prepend-icon="mdi-biohazard" label="Nome" :rules="[(v) => !!v ||
                                                'Nome obrigatório']" v-model="assignmentToAdd.a_name"></v-text-field>
        ...
        <v-btn @click="add = true" color="deep-orange" large right fixed bottom fab
